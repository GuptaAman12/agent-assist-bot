...
def kb_unmatched(limit: int = Query(default=50, ge=1, le=100)):
...
@app.get("/handoff/queue", dependencies=[Depends(require_admin)])
...
@app.post("/handoff/queue/replay", dependencies=[Depends(require_admin)])
...
@app.delete("/handoff/queue/{ticket_id}", dependencies=[Depends(require_admin)])
